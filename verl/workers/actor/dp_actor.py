...
class DataParallelPPOActor(BasePPOActor):
    ...
    def update_policy(self, data: DataProto):
        ...
        # Include OPD keys if present (for loss mode)
        ...
        for _ in range(self.config.ppo_epochs):
            for batch_idx, mini_batch in enumerate(mini_batches):
                ...
                for micro_batch in micro_batches:
                    ...
                    # Add OPD (On-Policy Distillation) KL loss (LOSS MODE ONLY)
                    # In loss mode, teacher guidance is added as separate loss term
                    # In advantage mode, this is skipped (handled in ray_trainer.py)
                    opd_config = getattr(self.config, "opd_config", None)
                    if (
                        opd_config is not None
                        and opd_config.get("opd_mode", "advantage") == "loss"  # Only in loss mode
                        and "opd_eligibility_mask" in model_inputs
                        and "opd_horizon_mask" in model_inputs
                        ...
                        if num_eligible > 0:
                            ...
                            opd_kl_loss = agg_loss(
                                loss_mat=kld_opd, loss_mask=opd_combined_mask.to(torch.bool), loss_agg_mode=loss_agg_mode
                            )
